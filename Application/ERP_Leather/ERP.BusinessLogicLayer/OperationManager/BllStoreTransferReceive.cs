...
{
    public class BllStoreTransferReceive
    {
         #region Private Variables

        private readonly DatabseManager _databseManager;
        private DalStoreTransferReceive _dalStoreTransferReceive;
        ...
        public BllStoreTransferReceive()
        {
        ...
        public ValidationMsg Save(INVStoreTrans model, int userId, string pageUrl)
        {
            ...
            using (var transactionScope = new TransactionScope(TransactionScopeOption.RequiresNew, transactionOption))
            {
                try
                {
                    _dalStoreTransferReceive = new DalStoreTransferReceive();

                    TransactionID = _dalStoreTransferReceive.Save(model, userId, pageUrl);
        ...
        public ValidationMsg Update(INVStoreTrans model, int userId)
        {
            ...
            using (var transactionScope = new TransactionScope(TransactionScopeOption.RequiresNew, transactionOption))
            {
                try
                {
                    ...
                    var UpdateStatus = _dalStoreTransferReceive.Update(model, userId);
