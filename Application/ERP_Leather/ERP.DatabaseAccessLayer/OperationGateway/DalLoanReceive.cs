    ...
    public class DalLoanReceive
    {
                            ...
                            if (model.InvStoreTransChallanList != null)
                            {
                                foreach (INVStoreTransChallan objInvStoreTransChallan in model.InvStoreTransChallanList)
                                {
                                    if (objInvStoreTransChallan.TransChallanID == 0)
                                    {
                                        ...
                                        objInvStoreTransChallan.ChallanDate = objInvStoreTransChallan.ChallanDate.Contains("/") ? objInvStoreTransChallan.ChallanDate : Convert.ToDateTime(objInvStoreTransChallan.ChallanDate).ToString("dd/MM/yyyy");
                                        objInvStoreTransChallan.TransChallanNo = DalCommon.GetPreDefineNextCodeByUrl("ChemicalConsumption/ChemConsumption");
                                        INV_StoreTransChallan tblChemLocalPurcRecvChallan = SetToModelObject(objInvStoreTransChallan, userid);
                                    ...
                                    else
                                    {
                                        ...
                                        var OrgrEntity = _context.INV_StoreTransChallan.First(m => m.TransChallanID == objInvStoreTransChallan.TransChallanID);
                        ...
                        if (model.InvStoreTransItemList != null)
                        {
                            foreach (var chemicalItem in model.InvStoreTransItemList)
                            {
                                ...
                                if (CheckDate)
                                {
                                    ...
                                    CurrentItem.Remark = "Loan Receive";
                                ...
                                else
                                {
                                    ...
                                    objStockDaily.Remark = "Loan Receive";
                                ...
                                if (!CheckSupplierStock)
                                {
                                    ...
                                    objStockSupplier.Remark = "Loan Receive";
                                ...
                                if (!CheckItemStock)
                                {
                                    ...
                                    objStockItem.Remark = "Loan Receive";
                            ...
                            var reqsId =
                                _context.INV_StoreTransRequest.Where(m => m.TransactionID == model.TransactionID)
                                    .FirstOrDefault()
                                    .RequestID;
                            ...
                            curChemFrgnPurcOrdrEntity.RecordStatus = "RCV";
        ...
        public List<INVStoreTransRequest> GetLoanRequestInfoList(string RequestID)
        {
            if (!string.IsNullOrEmpty(RequestID))
            {
                ...
                List<INV_TransRequest> searchList = _context.INV_TransRequest.Where(m => m.RecordStatus == "CNF" && m.RequestType == "LNRR" && m.RequestID == requestID).OrderByDescending(m => m.RequestID).ToList();
        ...
        {
            List<INV_StoreTrans> searchList = _context.INV_StoreTrans.Where(m => m.TransactionCategory == "RCV" && m.TransactionType == "LNR" && m.TransactionState == "FLR" && m.TransactionStatus == "TRI").OrderByDescending(m => m.TransactionID).ToList();
        ...
        {
            List<INV_TransRequest> searchList = _context.INV_TransRequest.Where(m => m.RequestType == "LNRR" && m.RecordStatus == "CNF").OrderByDescending(m => m.RequestID).ToList();
        ...
        public TransRequest SetToBussinessObjectLonRcvRqst(INV_TransRequest Entity)
        {
            ...
            {
                if (!string.IsNullOrEmpty(Entity.RequestTo))
                {
                    byte transactionFrom = Convert.ToByte(Entity.RequestTo);
                    Model.RequestToName = _context.SYS_Store.Where(m => m.StoreID == transactionFrom).FirstOrDefault().StoreName == null ? null : _context.SYS_Store.Where(m => m.StoreID == transactionFrom).FirstOrDefault().StoreName;
                }
            }
            else
            {
                if (!string.IsNullOrEmpty(Entity.RequestTo))
                {
                    int SupplierID = Convert.ToInt32(Entity.RequestTo);
                    Model.RequestToName = _context.Sys_Supplier.Where(m => m.SupplierID == SupplierID).FirstOrDefault().SupplierName == null ? null : _context.Sys_Supplier.Where(m => m.SupplierID == SupplierID).FirstOrDefault().SupplierName;
                }
            }

            Model.RequestTo = Entity.RequestTo;
            Model.RequestFrom = Entity.RequestFrom;
            byte transactionTo = Convert.ToByte(Entity.RequestFrom);
            Model.RequestFromName = _context.SYS_Store.Where(m => m.StoreID == transactionTo).FirstOrDefault().StoreName == null ? null : _context.SYS_Store.Where(m => m.StoreID == transactionTo).FirstOrDefault().StoreName;
        ...
        public ValidationMsg DeletedReceive(long? TransactionID)
        {
            _vmMsg = new ValidationMsg();
            try
            {
                var challanList = _context.INV_StoreTransChallan.Where(m => m.TransactionID == TransactionID).ToList();
                var itemList = _context.INV_StoreTransItem.Where(m => m.TransactionID == TransactionID).ToList();
                ...
                {
                    var deleteplElement = _context.INV_StoreTransRequest.First(m => m.TransactionID == TransactionID);
                    _context.INV_StoreTransRequest.Remove(deleteplElement);

                    var deleteElement = _context.INV_StoreTrans.First(m => m.TransactionID == TransactionID);
        ...
        public ValidationMsg DeletedReceiveChallan(long? TransChallanID)
        {
            _vmMsg = new ValidationMsg();
            try
            {
        ...
        public List<SysSupplier> GetSupplierSoruceList()
        {
            var query = @"select distinct cast(RequestTo as int) SupplierID,(select SupplierName from Sys_Supplier
                        ...
                        where ToSource = 'SUP' and RequestType='LNRR'";
        ...
        public List<SysStore> GetStoreSoruceList()
        {
            var query = @"select distinct cast(RequestTo as tinyint) StoreID,(select StoreName from SYS_Store
                        ...
                        where ToSource = 'STR' and RequestType='LNRR'";
