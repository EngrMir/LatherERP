...
{
    public class DalFinishLeatherBuyerQC
    {
        ...
        public DalFinishLeatherBuyerQC()
        {
        ...
        public PRD_FinishLeatherQC SetToModelObject(PrdCrustLeatherQC model, int userid)
        {
            ...
            Entity.QCTransactionOf = "FBQC"; //Buyer QC
                ...
                if (!string.IsNullOrEmpty(BuyerID))
                {
                    if (string.IsNullOrEmpty(BuyerOrderID))
                    {
                        var query = @"select inv.TransectionID,
                            ...
                            from dbo.INV_FinishBuyerQCStock inv
                            INNER JOIN (select MAX(TransectionID)TransectionID,BuyerID,StoreID,ItemTypeID,LeatherTypeID,LeatherStatusID,ArticleID,ColorID from dbo.INV_FinishBuyerQCStock
                            group by StoreID,BuyerID,BuyerOrderID,ItemTypeID,LeatherTypeID,LeatherStatusID,ArticleID,ColorID) sup
                ...
                if (!string.IsNullOrEmpty(BuyerID))
                {
                    if (string.IsNullOrEmpty(BuyerOrderID))
                    {
                        var query = @"select inv.TransectionID,
                            ...
                            inv.ClosingStockPcs ProductionPcs,inv.ClosingStockSide ProductionSide,inv.ClosingStockArea ProductionArea from dbo.INV_FinishBuyerQCStock inv
                            INNER JOIN (select MAX(TransectionID)TransectionID,BuyerID,StoreID,ItemTypeID,LeatherTypeID,LeatherStatusID,ArticleID,ColorID,GradeID from dbo.INV_FinishBuyerQCStock
                            group by StoreID,BuyerID,BuyerOrderID,ItemTypeID,LeatherTypeID,LeatherStatusID,ArticleID,ColorID,GradeID) sup
                            ON inv.TransectionID=sup.TransectionID
                            ...
                            " and inv.ColorID = " + ColorID + " and inv.ArticleChallanID = " + ArticleChallanID + " and (inv.ClosingStockPcs>0 or inv.ClosingStockSide>0 or inv.ClosingStockArea>0)";
        ...
        public PrdCrustLeatherQCSelection SetToBussinessObject(PRD_FinishLeatherQCSelection Entity, byte storeId)
        {
            ...
            Model.ProductionPcs = _context.INV_FinishBuyerQCStock.
                Where(ma => ma.StoreID == storeId
                    && ma.BuyerID == BuyerID
                    && ma.BuyerOrderID == BuyerOrderID
                    && ma.ItemTypeID == ItemTypeID
            ...
            Model.ProductionSide = _context.INV_FinishBuyerQCStock.
                Where(ma => ma.StoreID == storeId
                    && ma.BuyerID == BuyerID
                    && ma.BuyerOrderID == BuyerOrderID
                    && ma.ItemTypeID == ItemTypeID
            ...
            Model.ProductionArea = _context.INV_FinishBuyerQCStock.
                Where(ma => ma.StoreID == storeId
                    && ma.BuyerID == BuyerID
                    && ma.BuyerOrderID == BuyerOrderID
                    && ma.ItemTypeID == ItemTypeID
        ...
        {
            List<PRD_FinishLeatherQC> searchList = _context.PRD_FinishLeatherQC.Where(m => m.QCTransactionOf == "FBQC").OrderByDescending(m => m.FinishLeatherQCID).ToList();
        ...
        public PrdCrustLeatherQC SetToBussinessObject(PRD_FinishLeatherQC Entity)
        {
            ...
            if (Entity.QCTransactionOf == "FBQC")
            {
                Model.QCTransactionOfName = "Buyer QC";
                Model.QCTransactionOf = "Buyer QC";
            }
        ...
        public List<PrdCrustLeatherQCItem> GetScheduleItemInfo(string ProductionFloor)
        {
            if (!string.IsNullOrEmpty(ProductionFloor))
            {
                var query = @"select inv.TransectionID,
                            ...
                            group by StoreID,BuyerID,BuyerOrderID,ItemTypeID,LeatherTypeID,LeatherStatusID,ArticleID,ColorID) sup
                            ON inv.TransectionID=sup.TransectionID
                            where inv.StoreID = " + ProductionFloor + " and (inv.ClosingStockPcs>0 or inv.ClosingStockSide>0 or inv.ClosingStockArea>0)";
                var allData = _context.Database.SqlQuery<PrdCrustLeatherQCItem>(query).ToList();
                return allData;
            }
            return null;
                            ...
                            foreach (PrdCrustLeatherQCItem objQCItem in model.PrdCrustLeatherQCItemList)
                            {
                                if (model.PrdCrustLeatherQCSelectionList != null)
                                {
                                    foreach (PrdCrustLeatherQCSelection objQCSelection in model.PrdCrustLeatherQCSelectionList)
                                    {
                                        ...
                                        var CrustProductionStock = (from ds in _context.INV_FinishBuyerQCStock.AsEnumerable()
                                        ...
                                        {
                                            var LastItemInfo = (from ds in _context.INV_FinishBuyerQCStock
                                                                where ds.StoreID == model.ProductionFloor
                                                                && ds.BuyerID == objQCItem.BuyerID
                                                                && ds.BuyerOrderID == objQCItem.BuyerOrderID
                                                                && ds.ArticleID == objQCItem.ArticleID
                                            ...
                                            if ((LastItemInfo.ClosingStockPcs >= QCPcsQty) && (LastItemInfo.ClosingStockSide >= QCSideQty) && (LastItemInfo.ClosingStockArea >= QCAreaQty))
                                            {
                                                #region Production Completed

                                                var objPCMStockItem = new INV_FinishBuyerQCStock();
                                                ...
                                                if ((string.IsNullOrEmpty(objQCSelection.QCPassAreaUnitName)) && (string.IsNullOrEmpty(objQCSelection.QCFailAreaUnitName)))
                                                    objPCMStockItem.ClosingStockAreaUnit = null;
                                                else
                                                    objPCMStockItem.ClosingStockAreaUnit = !(string.IsNullOrEmpty(objQCSelection.QCPassAreaUnitName)) ? _context.Sys_Unit.Where(m => m.UnitName == objQCSelection.QCPassAreaUnitName).FirstOrDefault().UnitID : _context.Sys_Unit.Where(m => m.UnitName == objQCSelection.QCFailAreaUnitName).FirstOrDefault().UnitID;

                                                objPCMStockItem.FinishQCLabel = "PASS";

                                                objPCMStockItem.OpeningStockPcs = LastItemInfo.ClosingStockPcs;
                                                objPCMStockItem.ReceiveStockPcs = 0;
                                                objPCMStockItem.IssueStockPcs = (objQCSelection.QCPassPcs == null ? 0 : objQCSelection.QCPassPcs) + (objQCSelection.QCFailPcs == null ? 0 : objQCSelection.QCFailPcs);
                                                objPCMStockItem.ClosingStockPcs = LastItemInfo.ClosingStockPcs - ((objQCSelection.QCPassPcs == null ? 0 : objQCSelection.QCPassPcs) + (objQCSelection.QCFailPcs == null ? 0 : objQCSelection.QCFailPcs));

                                                objPCMStockItem.OpeningStockSide = LastItemInfo.ClosingStockSide;
                                                objPCMStockItem.ReceiveStockSide = 0;
                                                objPCMStockItem.IssueStockSide = (objQCSelection.QCPassSide == null ? 0 : objQCSelection.QCPassSide) + (objQCSelection.QCFailSide == null ? 0 : objQCSelection.QCFailSide);
                                                objPCMStockItem.ClosingStockSide = LastItemInfo.ClosingStockSide - ((objQCSelection.QCPassSide == null ? 0 : objQCSelection.QCPassSide) + (objQCSelection.QCFailSide == null ? 0 : objQCSelection.QCFailSide));

                                                objPCMStockItem.OpeningStockArea = LastItemInfo.ClosingStockArea;
                                                objPCMStockItem.ReceiveStockArea = 0;
                                                objPCMStockItem.IssueStockArea = (objQCSelection.QCPassArea == null ? 0 : objQCSelection.QCPassArea) + (objQCSelection.QCFailArea == null ? 0 : objQCSelection.QCFailArea);
                                                objPCMStockItem.ClosingStockArea = LastItemInfo.ClosingStockArea - ((objQCSelection.QCPassArea == null ? 0 : objQCSelection.QCPassArea) + (objQCSelection.QCFailArea == null ? 0 : objQCSelection.QCFailArea));

                                                _context.INV_FinishBuyerQCStock.Add(objPCMStockItem);
                                                _context.SaveChanges();
                                        ...
                                        if ((objQCSelection.QCPassPcs != null) || (objQCSelection.QCPassSide != null))
                                        {
                                            ...
                                            var CheckDate2 = (from ds in _context.INV_FinishPackingStock.AsEnumerable()
                                                              where ds.StockDate == currentDate
                                                                          && ds.BuyerID == objQCItem.BuyerID
                                                                          && ds.BuyerOrderID == objQCItem.BuyerOrderID
                                                                          && ds.StoreID == model.AfterQCIssueFloor
                                                                          && ds.ArticleID == objQCItem.ArticleID
                                                                          && ds.ArticleChallanID == objQCItem.ArticleChallanID
                                                                          && ds.ItemTypeID == objQCItem.ItemTypeID
                                                                          && ds.LeatherStatusID == objQCItem.LeatherStatusID
                                            ...
                                            if (CheckDate2)
                                            {
                                                var CurrentItem = (from ds in _context.INV_FinishPackingStock.AsEnumerable()
                                                                   where ds.StockDate == currentDate
                                                                         && ds.BuyerID == objQCItem.BuyerID
                                                                          && ds.BuyerOrderID == objQCItem.BuyerOrderID
                                                                          && ds.StoreID == model.AfterQCIssueFloor
                                                ...
                                                CurrentItem.IssueStockPcs = CurrentItem.ReceiveStockPcs + (objQCSelection.QCPassPcs ?? 0);
                                                CurrentItem.ClosingStockPcs = CurrentItem.ClosingStockPcs + (objQCSelection.QCPassPcs ?? 0);

                                                CurrentItem.IssueStockSide = CurrentItem.ReceiveStockSide + (objQCSelection.QCPassSide ?? 0);
                                                CurrentItem.ClosingStockSide = CurrentItem.ClosingStockSide + (objQCSelection.QCFailSide ?? 0);

                                                CurrentItem.IssueStockArea = CurrentItem.ReceiveStockArea + (objQCSelection.QCPassArea ?? 0);
                                                CurrentItem.ClosingStockPcs = CurrentItem.ClosingStockArea + (objQCSelection.QCPassArea ?? 0);
                                                CurrentItem.FinishQCLabel = "BQP";//Buyer QC Pass
                                                _context.SaveChanges();
                                            }
                                            else
                                            {
                                                var PreviousRecord = (from ds in _context.INV_FinishPackingStock.AsEnumerable()
                                                                      where ds.StoreID == model.AfterQCIssueFloor
                                                                            && ds.BuyerID == objQCItem.BuyerID
                                                                          && ds.BuyerOrderID == objQCItem.BuyerOrderID
                                                                          && ds.ArticleID == objQCItem.ArticleID
                                                                          && ds.ArticleChallanID == objQCItem.ArticleChallanID
                                                ...
                                                objStockDaily.StoreID = Convert.ToByte(model.AfterQCIssueFloor);
                                                ...
                                                objStockDaily.ColorID = Convert.ToInt32(objQCItem.ColorID);
                                                ...
                                                if (string.IsNullOrEmpty(objQCSelection.QCPassAreaUnitName))
                                                    ...
                                                    objStockDaily.ClosingStockAreaUnit = _context.Sys_Unit.Where(m => m.UnitName == objQCSelection.QCPassAreaUnitName).FirstOrDefault().UnitID;

                                                objStockDaily.OpeningStockPcs = (PreviousRecord == null ? 0 : PreviousRecord.ClosingStockPcs);
                                                objStockDaily.ReceiveStockPcs = objQCSelection.QCPassPcs ?? 0;
                                                objStockDaily.IssueStockPcs = 0;
                                                objStockDaily.ClosingStockPcs = objStockDaily.OpeningStockPcs + objQCSelection.QCPassPcs ?? 0;

                                                objStockDaily.OpeningStockSide = (PreviousRecord == null ? 0 : PreviousRecord.ClosingStockSide);
                                                objStockDaily.ReceiveStockSide = objQCSelection.QCPassSide ?? 0;
                                                objStockDaily.IssueStockSide = 0;
                                                objStockDaily.ClosingStockSide = objStockDaily.OpeningStockSide + objQCSelection.QCPassSide ?? 0;

                                                objStockDaily.OpeningStockArea = (PreviousRecord == null ? 0 : PreviousRecord.ClosingStockArea);
                                                objStockDaily.ReceiveStockArea = objQCSelection.QCPassArea ?? 0;
                                                objStockDaily.IssueStockArea = 0;
                                                objStockDaily.ClosingStockArea = objStockDaily.OpeningStockArea + objQCSelection.QCPassArea ?? 0;
                                        ...
                                        if ((objQCSelection.QCFailPcs != null) || (objQCSelection.QCFailSide != null))
                                        {
                                            ...
                                            if (CheckDate2)
                                            {
                                                ...
                                                CurrentItem.FinishQCLabel = "BQF";//Buyer QC Fail
                                                _context.SaveChanges();
                                            }
                                            else
                                            {
                                                ...
                                                objStockDaily.FinishQCLabel = "BQF";//Buyer QC Fail
                                            ...
                                            if (!CheckBuyerStock2)
                                            {
                                                ...
                                                objStockSupplier.FinishQCLabel = "BQF";//Buyer QC Fail
                                            ...
                                            if (!CheckItemStock2)
                                            {
                                                ...
                                                objStockItem.FinishQCLabel = "BQF";//Buyer QC Fail
                ...
                if (!string.IsNullOrEmpty(BuyerID))
                {
                    if (string.IsNullOrEmpty(BuyerOrderID))
                    {
                        var query = @"select inv.TransectionID,inv.GradeRange,
                            ...
                            group by StoreID,BuyerID,BuyerOrderID,ItemTypeID,LeatherTypeID,LeatherStatusID,ArticleID,ColorID,GradeID) sup
                            ON inv.TransectionID=sup.TransectionID
                     where inv.StoreID = " + ProductionFloor + " and inv.BuyerID = " + BuyerID + " and inv.BuyerOrderID IS NULL and inv.ItemTypeID = " + ItemTypeID + " and inv.LeatherStatusID = " + LeatherStatusID + " and inv.ArticleID = " + ArticleID + " and inv.ColorID = " + ColorID + "  and inv.ArticleChallanID = " + ArticleChallanID + " and (inv.ClosingStockPcs>0 or inv.ClosingStockSide>0 or inv.ClosingStockArea>0)";
