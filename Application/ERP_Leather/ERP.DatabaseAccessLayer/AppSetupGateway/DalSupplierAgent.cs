    ...
    public class DalSupplierAgent
    {
            ...
            try
            {
                using (var tx = new TransactionScope())
                {
                    using (_context)
                    {
                        ...
                        //{
                        #region Supplier
                        model.SupplierCode = DalCommon.GetPreDefineNextCodeByUrl("SupplierAgent/SupplierAgent");//DalCommon.GetPreDefineValue("1", "00045");
        ...
        public ValidationMsg Update(SysSupplier model, int userid)
        {
            ...
            try
            {
                using (var tx = new TransactionScope())
                {
                    using (_context)
                    {
                        ...
                        OriginalEntity.CountryID = CurrentEntity.CountryID;
            ...
            catch (Exception ex)
            {
                ...
                {
                    _vmMsg.Type = Enums.MessageType.Error;
                    _vmMsg.Msg = "Agent Code Already Exit..";
                }
                else
                {
                    _vmMsg.Type = Enums.MessageType.Error;
        ...
        public List<SupplierDetails> GetAllChemicalSupplier()
        {
            var allData = (from s in _context.Sys_Supplier
                           where s.IsActive.Equals(true) && s.IsDelete.Equals(false) && s.SupplierCategory == "Chemical"
                           join sa in _context.Sys_SupplierAddress.Where(q => q.IsActive.Equals(true) && q.IsDelete.Equals(false))
        ...
        public List<SupplierDetails> GetChemicalSupplierByType(string type)
        {
            var allData = (from s in _context.Sys_Supplier
        ...
        {
            List<Sys_Supplier> searchList = _context.Sys_Supplier.Where(m => !m.IsDelete && m.SupplierType.Contains("Agent")).ToList();
