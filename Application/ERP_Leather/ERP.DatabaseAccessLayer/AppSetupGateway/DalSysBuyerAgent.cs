...
namespace ERP.DatabaseAccessLayer.AppSetupGateway
{
    public class DalSysBuyerAgent
    {
                        ...
                        else
                        {
                            ...
                            if (model.BuyerAddressList != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        _vmMsg.Type = Enums.MessageType.Error;
                                        _vmMsg.Msg = "Please Enter only one Active Buyer Agent Address.";
                                    }
                                    //if (buyeraddress == 0)
                                    else
                                    {
        ...
        {
            List<Sys_Buyer> searchList = _context.Sys_Buyer.Where(m => m.BuyerCategory == "Buyer Agent" || m.BuyerCategory == "CNF Agent").ToList();
        ...
        public List<string> GetBuyerListForSearch()
        {
            ...
            return _context.Sys_Buyer.Where(m => m.BuyerType == "Local Agent" || m.BuyerType == "Foreign Agent").Select(m => m.BuyerName).ToList();
        }
